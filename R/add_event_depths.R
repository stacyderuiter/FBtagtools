...
#' @param z depth data: sensor data structure, vector, matrix, tibble, or data frame. If not a sensor structure, then `sampling_rate` is required. If tibble or df it should have only one column (depth data).
...
add_event_depths <- function(x, start_x,
  ...
  if (utils::hasName(z, 'data')){ # if x is sensor data structure}
    ...
    z <- as.numeric(z$data) # z is now a vector or matrix
  }
  ...
  x <- x %>%
    dplyr::mutate(event_depth = as.numeric(event_depth))
