  ...
  for (t in c(1:length(tags))){
    ...
    if (nrow(this_allclicks) > 0){
      for (d in c(1:nrow(these_dives))){
        focal_dive_clicks <- this_focal_clicks %>%
          dplyr::filter(sec_since_tagon > these_dives$start[d] &
                          sec_since_tagon <= these_dives$end[d])
        nf_dive_clicks <- this_nf_clicks %>%
          dplyr::filter(sec_since_tagon > these_dives$start[d] &
                          sec_since_tagon <= these_dives$end[d])
        dive_buzzes <- this_buzz %>%
          dplyr::filter(sec_since_tagon > these_dives$start[d] &
                          sec_since_tagon <= these_dives$end[d])


        dive_depth <- this_data$depth$data[c(floor(these_dives$start[d] *
                                                      this_data$depth$sampling_rate) :
                                                ceiling(these_dives$end[d] *
                                                          this_data$depth$sampling_rate))]
##############################
        # need to pre-allocate space here
        if (nrow(focal_dive_clicks) > 0){
          these_dives$click_dur_sec[d] <- pull(focal_dive_clicks,
                                             sec_since_tagon) %>%
          range(na.rm = TRUE) %>% diff()

          ck_start_cst <- pull(focal_dive_clicks,
                               sec_since_tagon) %>%
            min(na.rm = TRUE)
          ck_end_cst <- pull(focal_dive_clicks,
                             sec_since_tagon) %>%
            max(na.rm = TRUE)

          click_depth <- this_data$depth$data[c(floor(min(focal_dive_clicks$sec_since_tagon,
                                                          na.rm = TRUE) *
                                                        this_data$depth$sampling_rate) :
                                                  ceiling(max(focal_dive_clicks$sec_since_tagon,
                                                              na.rm = TRUE) *
                                                            this_data$depth$sampling_rate))]

          these_dives$dive_to_click1_sec[d] <-  ck_start_cst -
            these_dives$start[d]
          these_dives$first_click_depth[d] <- this_data$depth$data[round((ck_start_cst -
                                                                    this_data$depth$start_offset)*
                                                                   this_data$depth$sampling_rate)]
          these_dives$last_click_depth[d] <- this_data$depth$data[round((ck_end_cst -
                                                                         this_data$depth$start_offset)*
                                                                        this_data$depth$sampling_rate)]
          these_dives$min_click_depth[d] <- min(click_depth, na.rm = TRUE)
          these_dives$max_click_depth[d] <- max(click_depth, na.rm = TRUE)
          these_dives$n_clicks[d] <- nrow(focal_dive_clicks)
          these_dives$percent_dive_clicking[d] <- these_dives$click_dur_sec[d] /
            these_dives$dive_dur_sec[d] * 100

        } # end of "if there are focal clicks this dive"
        these_dives$other_bw_clicks[d] <- ifelse(nrow(nf_dive_clicks) > 0,
                                                 'present',
                                                 'absent')

        if (nrow(dive_buzzes) > 0){
          these_dives$n_buzzes[d] <- nrow(dive_buzzes)
          these_dives$min_buzz_depth[d] <- min(dive_buzzes$buzz_depth, na.rm = TRUE)
          these_dives$max_buzz_depth[d] <- max(dive_buzzes$buzz_depth, na.rm = TRUE)
          these_dives$mean_buzz_depth[d] <- mean(dive_buzzes$buzz_depth, na.rm = TRUE)
          these_dives$median_buzz_depth[d] <- median(dive_buzzes$buzz_depth, na.rm = TRUE)

          # if there are any buzzes this dive
        }# end if there are any buzzes this dive

    } # end loop over dives
    } # end if there are any clicks this tagout
