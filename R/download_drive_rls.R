#' Download file with (rawest, confusing) received levels of anthropogenic sounds from the FB Google Drive
#'
#' This function downloads the "raw" format of RL data, for all types of anthropogenic sounds, in all available variations of RL type and frequency band. Most users will want to also use \code{\link[FBtagtools]{parse_rls}} to generate data frames or text files with data in a simpler format.
#'
#' @param rl_file name of .csv file (on Google Drive) with raw RL data. Defaults to: qPing_log_corr_times_master.csv (most users should not need to change this, and at present there IS no other file.)
...
download_drive_rls <- function(rl_file = 'qPing_log_corr_times_master',
                                  path = getwd(),
                              email,
                              overwrite = TRUE){
  if (missing(email)){
